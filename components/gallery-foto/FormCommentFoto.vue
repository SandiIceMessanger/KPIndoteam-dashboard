<template>
  <div>
    ...
    <modal name="modalAgreeGalleriFoto">
      <client-only>
        <div id="form-login">
...
<script>
  ...
  import ModalTnc from '~/components/notice/ModalTnc';
  ...
  export default {
    ...
    methods: {
      ...
      hideModal() {
        this.$modal.hide('modalAgreeGalleriFoto')
      },
      async onShowNotice() {
        if (this.$store.state.auth.loggedIn == false) {
          this.openModalLogin()
          ...
          if(this.$store.state.comments.checkAgree == 'agree'){
            this.$modal.show('modalAgreeGalleriFoto')
          }else{
            this.$modal.show('modalTncGallyFoto')
          }
        }
      },
      async agree() {
        ...
        if (this.$store.state.comments.checkAgree == 'agree') {
          this.$modal.hide('modalTncGallyFoto')
          this.$modal.show('modalAgreeGalleriFoto')
        }else{
          this.$modal.hide('modalTncGallyFoto')
          this.$modal.hide('modalAgreeGalleriFoto')
